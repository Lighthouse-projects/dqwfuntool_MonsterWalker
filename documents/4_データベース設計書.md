...
| strategy_type | strategy_type_enum | NOT NULL | 攻略タイプ（oneshot/semiauto/auto/manual） |
...
**削除方式**: 物理削除（CASCADE削除で関連データも自動削除）
...
**DDL**:
```sql
CREATE TYPE strategy_type_enum AS ENUM ('oneshot', 'semiauto', 'auto', 'manual');
...
CREATE POLICY "Strategies are viewable by everyone" ON mw_strategies FOR SELECT USING (true);
CREATE POLICY "Users can insert their own strategies" ON mw_strategies FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update their own strategies" ON mw_strategies FOR UPDATE USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
...
| content | text | NOT NULL | 要望内容（最大500文字） |
| status | request_status_enum | NOT NULL, DEFAULT 'pending' | 対応状況 |
...
CREATE INDEX idx_mw_requests_category ON mw_requests (category);
CREATE INDEX idx_mw_requests_status ON mw_requests (status);
...
erDiagram
    ...
    mw_requests {
        ...
        uuid user_id FK
        request_category_enum category
        text content
...
4. **作成日時** (`idx_mw_strategies_created_at`)
   - 新着順ソートに使用
