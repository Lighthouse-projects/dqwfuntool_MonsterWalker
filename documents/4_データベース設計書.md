...
CREATE TABLE profiles (
  ...
  x_account_name varchar(50) NULL,
...
| カラム名 | データ型 | 制約 | 説明 |
|---------|---------|------|------|
| id | bigint | PK, GENERATED ALWAYS AS IDENTITY | NGワードID |
...
| カラム名 | データ型 | 制約 | 説明 |
|---------|---------|------|------|
| id | bigint | PK, GENERATED ALWAYS AS IDENTITY | モンスターID |
| name | varchar(100) | NOT NULL, UNIQUE | モンスター名 |
...
| カラム名 | データ型 | 制約 | 説明 |
|---------|---------|------|------|
| id | bigint | PK, GENERATED ALWAYS AS IDENTITY | 武器ID |
| name | varchar(100) | NOT NULL, UNIQUE | 武器名 |
| sort_order | int | NOT NULL, DEFAULT 0 | 表示順 |
| is_active | boolean | NOT NULL, DEFAULT true | 有効フラグ |
| created_at | timestamptz | NOT NULL, DEFAULT now() | 登録日時 |
| updated_at | timestamptz | NOT NULL, DEFAULT now() | 更新日時 |
...
| カラム名 | データ型 | 制約 | 説明 |
|---------|---------|------|------|
| id | bigint | PK, GENERATED ALWAYS AS IDENTITY | 職業ID |
| name | varchar(100) | NOT NULL, UNIQUE | 職業名 |
| sort_order | int | NOT NULL, DEFAULT 0 | 表示順 |
| is_active | boolean | NOT NULL, DEFAULT true | 有効フラグ |
| created_at | timestamptz | NOT NULL, DEFAULT now() | 登録日時 |
| updated_at | timestamptz | NOT NULL, DEFAULT now() | 更新日時 |
...
| カラム名 | データ型 | 制約 | 説明 |
|---------|---------|------|------|
| id | bigint | PK, GENERATED ALWAYS AS IDENTITY | 攻略情報ID |
| user_id | uuid | NOT NULL, FK → auth.users(id) ON DELETE CASCADE | 登録ユーザーID |
| monster_id | bigint | NOT NULL, FK → mw_mst_monsters(id) | 攻略対象モンスターID |
| strategy_type | varchar(20) | NOT NULL | 攻略タイプ（oneshot/semiauto/auto） |
...
CREATE TABLE mw_strategies (
  ...
  monster_id bigint NOT NULL REFERENCES mw_mst_monsters(id),
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT check_strategy_type CHECK (strategy_type IN ('oneshot', 'semiauto', 'auto'))
);

-- インデックス作成
CREATE INDEX idx_mw_strategies_user_id ON mw_strategies (user_id);
CREATE INDEX idx_mw_strategies_monster_id ON mw_strategies (monster_id);
...
| カラム名 | データ型 | 制約 | 説明 |
|---------|---------|------|------|
| id | bigint | PK, GENERATED ALWAYS AS IDENTITY | メンバーID |
| strategy_id | bigint | NOT NULL, FK → mw_strategies(id) ON DELETE CASCADE | 攻略情報ID |
| member_order | int | NOT NULL | メンバー順番（1〜4） |
| weapon_id | bigint | NOT NULL, FK → mw_mst_weapons(id) | 武器ID |
| job_id | bigint | NOT NULL, FK → mw_mst_jobs(id) | 職業ID |
...
CREATE TABLE mw_strategy_members (
  id bigint PRIMARY KEY GENERATED ALWAYS AS IDENTITY,
  strategy_id bigint NOT NULL REFERENCES mw_strategies(id) ON DELETE CASCADE,
  member_order int NOT NULL,
  weapon_id bigint NOT NULL REFERENCES mw_mst_weapons(id),
  job_id bigint NOT NULL REFERENCES mw_mst_jobs(id),
  screenshot_front_url text NOT NULL,
  screenshot_back_url text NOT NULL,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT unique_strategy_member_order UNIQUE (strategy_id, member_order),
  CONSTRAINT check_member_order CHECK (member_order >= 1 AND member_order <= 4)
);

-- インデックス作成
CREATE INDEX idx_mw_strategy_members_strategy_id ON mw_strategy_members (strategy_id);
...
CREATE POLICY "Users can insert their own strategy members" ON mw_strategy_members FOR INSERT
  WITH CHECK (EXISTS (SELECT 1 FROM mw_strategies WHERE id = strategy_id AND user_id = auth.uid()));
CREATE POLICY "Users can update their own strategy members" ON mw_strategy_members FOR UPDATE
  USING (EXISTS (SELECT 1 FROM mw_strategies WHERE id = strategy_id AND user_id = auth.uid()));
...
| カラム名 | データ型 | 制約 | 説明 |
|---------|---------|------|------|
| id | bigint | PK, GENERATED ALWAYS AS IDENTITY | いいねID |
...
CREATE TABLE mw_likes (
  ...
  strategy_id bigint NOT NULL REFERENCES mw_strategies(id) ON DELETE CASCADE,
  created_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT unique_user_strategy_like UNIQUE (user_id, strategy_id)
);

-- インデックス作成
CREATE UNIQUE INDEX idx_mw_likes_unique ON mw_likes (user_id, strategy_id);
CREATE INDEX idx_mw_likes_strategy_id ON mw_likes (strategy_id);
...
| カラム名 | データ型 | 制約 | 説明 |
|---------|---------|------|------|
| id | bigint | PK, GENERATED ALWAYS AS IDENTITY | お気に入りID |
| user_id | uuid | NOT NULL, FK → auth.users(id) ON DELETE CASCADE | ユーザーID |
| strategy_id | bigint | NOT NULL, FK → mw_strategies(id) ON DELETE CASCADE | 攻略情報ID |
| sort_order | int | NOT NULL, DEFAULT 0 | 表示順 |
| created_at | timestamptz | NOT NULL, DEFAULT now() | 登録日時 |
...
CREATE TABLE mw_favorites_strategies (
  ...
  sort_order int NOT NULL DEFAULT 0,
  created_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT unique_user_strategy_favorite UNIQUE (user_id, strategy_id)
);

-- インデックス作成
CREATE UNIQUE INDEX idx_mw_favorites_strategies_unique ON mw_favorites_strategies (user_id, strategy_id);
...
| id | bigint | PK, GENERATED ALWAYS AS IDENTITY | お気に入りID |
| user_id | uuid | NOT NULL, FK → auth.users(id) ON DELETE CASCADE | ユーザーID |
| monster_id | bigint | NULL, FK → mw_mst_monsters(id) | モンスターID（検索条件） |
| weapon_id | bigint | NULL, FK → mw_mst_weapons(id) | 武器ID（検索条件） |
...
CREATE TABLE mw_favorites_searches (
  id bigint PRIMARY KEY GENERATED ALWAYS AS IDENTITY,
  user_id uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  monster_id bigint NULL REFERENCES mw_mst_monsters(id),
  weapon_id bigint NULL REFERENCES mw_mst_weapons(id),
...
| カラム名 | データ型 | 制約 | 説明 |
|---------|---------|------|------|
| id | bigint | PK, GENERATED ALWAYS AS IDENTITY | 通報ID |
| reporter_user_id | uuid | NOT NULL, FK → auth.users(id) ON DELETE CASCADE | 通報者ユーザーID |
| strategy_id | bigint | NOT NULL, FK → mw_strategies(id) ON DELETE CASCADE | 通報対象攻略情報ID |
...
CREATE TABLE mw_reports (
  ...
  strategy_id bigint NOT NULL REFERENCES mw_strategies(id) ON DELETE CASCADE,
  created_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT unique_reporter_strategy UNIQUE (reporter_user_id, strategy_id)
);

-- インデックス作成
CREATE UNIQUE INDEX idx_mw_reports_unique ON mw_reports (reporter_user_id, strategy_id);
CREATE INDEX idx_mw_reports_strategy_id ON mw_reports (strategy_id);
...
5. mw_mst_jobs テーブル
6. mw_strategies テーブル
7. mw_strategy_members テーブル
8. mw_likes テーブル
9. mw_favorites_strategies テーブル
10. mw_favorites_searches テーブル
11. mw_reports テーブル

※外部キー制約の依存関係により、この順序で実行する必要があります。
...
2. **モンスター検索** (`idx_mw_strategies_monster_id`)
   - モンスター別の攻略情報検索に使用
