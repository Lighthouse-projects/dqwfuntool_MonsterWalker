...
| テーブル名 | 説明 | RLSポリシー |
|-----------|------|-------------|
| users | ユーザー情報 | 自分のデータのみ更新可、参照は全員可 |
| profiles | プロフィール情報（ニックネーム、アイコン、Xアカウント名） | 自分のデータのみ更新可、参照は全員可 |
| monsters | モンスターマスタ | 参照のみ（管理アプリから更新） |
| weapons | 武器マスタ | 参照のみ（管理アプリから更新） |
| jobs | 職業マスタ | 参照のみ（管理アプリから更新） |
| strategies | 攻略情報 | 自分のデータのみCRUD可、参照は全員可 |
| strategy_members | パーティメンバー情報（4人分） | strategiesに従属 |
| likes | いいね | 自分のいいねのみCRUD可 |
| favorites_strategies | 攻略情報お気に入り | 自分のお気に入りのみCRUD可 |
| favorites_searches | 検索条件お気に入り | 自分のお気に入りのみCRUD可 |
| reports | 通報 | 自分の通報のみ参照・登録可、更新・削除不可 |
| ng_words | NGワードリスト | 参照のみ（管理アプリから更新） |
